...
3. **CRITICAL STEP FOR EMULATORS:** You must configure the app to talk to your local machine's database. Android Emulators cannot use localhost; they use a specific alias IP (10.0.2.2) to access the host machine's local server.  
   You must open your code editor and change the Base API URL to http://10.0.2.2/masar-backend/ inside these **3 specific frontend files**:  
   * *File 1: (Insert your first file path here, e.g., src/api/config.js)*  
   * *File 2: (Insert your second file path here)*  
   * *File 3: (Insert your third file path here)*
